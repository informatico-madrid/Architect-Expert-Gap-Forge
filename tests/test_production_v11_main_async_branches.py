...
def test_main_async_resume_with_pending_frags(tmp_path, mock_args, mock_frags, monkeypatch):
    ...
    async def _test():
        ...
        # Mock dependencies
        monkeypatch.setattr(pv11, "load_taxonomy", MagicMock(return_value={"prompts": {"system": {"theory": "system"}, "user": {"theory": "user"}}}))
        monkeypatch.setattr(pv11, "load_master_docs", MagicMock(return_value=("master", "changelog", "jinja")))
        monkeypatch.setattr(pv11, "get_theory_fragments", MagicMock(return_value=mock_frags))
        monkeypatch.setattr(pv11, "load_checkpoint", MagicMock(return_value={"frag1_file1.py_0"}))  # one done
        mock_writer = MagicMock()
        mock_writer.write = AsyncMock()
        monkeypatch.setattr(pv11, "AsyncFileWriter", MagicMock(return_value=mock_writer))
        mock_tracker = MagicMock()
        mock_tracker.record = AsyncMock()
        mock_tracker.close = MagicMock()
        monkeypatch.setattr(pv11, "ProgressTracker", MagicMock(return_value=mock_tracker))

        mock_client = MagicMock()
        monkeypatch.setattr("src.factory.production_v11.AsyncOpenAI", MagicMock(return_value=mock_client))
...
def test_main_async_resume_all_processed(tmp_path, mock_args, mock_frags, monkeypatch):
    ...
    async def _test():
        ...
        monkeypatch.setattr(pv11, "load_taxonomy", MagicMock(return_value={"prompts": {}}))
...
def test_main_async_theory_mode(tmp_path, mock_args, mock_frags, monkeypatch):
    ...
    async def _test():
        ...
        monkeypatch.setattr(pv11, "load_taxonomy", MagicMock(return_value={"prompts": {}}))
        monkeypatch.setattr(pv11, "_TAX", {"prompts": {"theory": {"system": "system", "user": "user"}}})
        monkeypatch.setattr(pv11, "load_master_docs", MagicMock(return_value=("master", "changelog", "jinja")))
        monkeypatch.setattr(pv11, "get_theory_fragments", MagicMock(return_value=mock_frags))
        mock_writer = MagicMock()
        mock_writer.write = AsyncMock()
...
""")
        ...
        # Mock dependencies
        monkeypatch.setattr(pv11, "load_taxonomy", MagicMock(return_value={"prompts": {}}))
        monkeypatch.setattr(pv11, "_TAX", {"prompts": {"system": {"python": {"base": "system"}}, "user": {"nominal": {"code": "user"}}}})
        monkeypatch.setattr(pv11, "load_master_docs", MagicMock(return_value=("master", "changelog", "jinja")))
        monkeypatch.setattr(pv11, "detect_legacy_patterns", MagicMock(return_value=[]))  # no legacy
        mock_writer = MagicMock()
        mock_writer.write = AsyncMock()
        monkeypatch.setattr(pv11, "AsyncFileWriter", MagicMock(return_value=mock_writer))
        mock_tracker = MagicMock()
        ...
        mock_generate = AsyncMock(return_value={"status": "accepted", "sample": {"metadata": {"example_type": "nominal"}, "conversation": []}})
