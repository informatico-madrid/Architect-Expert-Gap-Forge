...
with tempfile.TemporaryDirectory() as tmp:
    ...
    # Process - repo_root passed to _emit_module is owner_dir (myrepo)
    ...
    # Simulate what _process_repository does
    repo_name = "myrepo"
    repo_path = owner_dir  # This is what gets passed to _emit_module as repo_root
    prefix = f"owner_{repo_name}"
    ...
    print(f'\nProcessing module with repo_root={repo_path}')
    for mod in modules:
        print(f'  Module: {mod.name}')
        for mf in mod.files:
            print(f'    - {mf.path.name} (role={mf.role}, size={mf.size})')
            if mf.role != "test" and mf.role != "anchor":
                # This is what _emit_module does
                test_file = find_test(mf.path, repo_path, MAX_SIZE_BACKEND)
                print(f'      find_test(repo_root={repo_path}): {test_file}')

    # Now run the full processor
    processor.run()
