...
| Observability metrics | ✅ Complete | `src/utils/metrics.py` |
| Benchmarking scripts | ✅ Complete | `scripts/benchmark/measure_performance.py` |
...
- **Script:** `scripts/benchmark/measure_performance.py`
- **Integration test:** `tests/integration/test_benchmark_compare.py`
- Run: `python scripts/benchmark/measure_performance.py`
...
- [ ] Header check passes: `python scripts/check_headers.py --check`
- [ ] Extractor adapters correctly parse Python files
- [ ] Git resilience handles network errors with exponential backoff
- [ ] Rate limit backoff respects `X-RateLimit-Reset` headers
- [ ] Parse error policy defaults to `abort` (marks `needs_manual_review`)
