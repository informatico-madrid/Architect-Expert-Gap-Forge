...
class LDIAnalyzer:
    ...
    def count_code_tokens(text: str) -> int:
        ...
        code_tokens += sum(
            1 for word in words if word in LDIAnalyzer.PROGRAMMING_KEYWORDS
        )
...
def analyze_false_negatives(gold: List[Dict], platinum: List[Dict]) -> None:
    ...
    print(
        f"📊 Muestras descartadas con razonamiento >1000 chars: {len(false_negatives)}"
    )
    ...
    for i, fn in enumerate(false_negatives[:10], 1):
        ...
        print(
            f"   LDI: {fn['ldi']} (código: {fn['code_tokens']}, texto: {fn['natural_tokens']})"
        )
    ...
    print(
        "   Propuesta: LDI 3.0 debe medir densidad solo en el tool_call, no en <think>"
    )
    print("=" * 80)
    ...
    print(
        f"   - Tasa potencial con LDI 3.0: {100 * (len(platinum) + len(false_negatives)) / len(gold):.1f}%"
    )
