        ...
        try:
            ...
            for rr in sample:
                try:
                    ...
                    if rid == ARGILLA_RECORD_ID or (
                        isinstance(rr, dict)
                        and rr.get("metadata", {}).get("sample_id") == SAMPLE_ID
                    ):
                        found = True
                        record_obj = rr
                        break
                    ...
                    if hasattr(d, "records"):
                        ...
                        try:
                            ...
                            for rr in sample2:
                                if (
                                    isinstance(rr, dict)
                                    and rr.get("metadata", {}).get("sample_id")
                                    == SAMPLE_ID
                                ):
                                    record_obj = rr
                                    found = True
...
if found:
    ...
    try:
        if hasattr(record_obj, "to_dict"):
            print(
                json.dumps(record_obj.to_dict(), ensure_ascii=False, indent=2)[:10000]
            )
            ...
            try:
                print(
                    json.dumps(record_obj.__dict__, ensure_ascii=False, indent=2)[
                        :10000
                    ]
                )
            except Exception:
