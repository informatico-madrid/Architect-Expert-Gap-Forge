...
2. **Missing prompt keys**: `_prompt("system.php_legacy.context")` is called in `prompt_builder.py:691` but `php_legacy/` has NO `prompts_taxonomy.yaml` file -- only `master_symfony_hex.md` and `snippets/`. This `_prompt()` call WILL fail at runtime (no fallback path resolves `system.php_legacy.context`). This is a **pre-existing bug** that the spec should flag rather than ignore.
