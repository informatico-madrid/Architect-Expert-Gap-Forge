...
- Q: ¿Cómo se resuelve la contradicción entre compatibilidad 100% Stage 2 (FR-004) y las extensiones IMPLICIT_DEPS/[LEGACY_SIGNATURES]? → A: Evolución del contrato — Stage 2 se actualiza con cambios mínimos: parser genérico de secciones (`\[(\w+)\]`), routing por extensión de archivo (eliminar AST lock), e inyección de [LEGACY_SIGNATURES] en el prompt del Teacher. Se añade categoría `MODERN_HYBRID` para código PHP híbrido (OOP + procedural). Retrocompatible con bundles Python existentes.
...
El driver clasifica cada patrón regex detectado en una categoría semántica del enum `SIGNATURE_CATEGORY`: `PERSISTENCE_SMELL` (acceso a BD), `STATE_POLLUTION` (estado global/sesión), `MODULE_LINK_SMELL` (dependencias de includes), `SECURITY_VULN` (vectores de inyección), `CONSTANT_POLLUTION` (defines/constantes), y `MODERN_HYBRID` (OOP híbrido). Estas categorías se incluyen en una sección [LEGACY_SIGNATURES] dentro de cada bundle, proporcionando al Teacher contexto sobre la naturaleza del código legacy.
...
1. **Given** un archivo PHP con llamadas `mysql_query()`, `tep_db_query()`, y `$wpdb->query()`, **When** el driver lo procesa, **Then** la sección [LEGACY_SIGNATURES] lista cada match con categoría `PERSISTENCE_SMELL`.
2. **Given** un archivo PHP con `global $currencies`, `$_SESSION['cart']`, y `tep_session_register()`, **When** el driver lo procesa, **Then** la sección [LEGACY_SIGNATURES] lista cada match con categoría `STATE_POLLUTION`.
3. **Given** un archivo PHP con `include(DIR_WS_INCLUDES . 'header.php')`, **When** el driver lo procesa, **Then** la sección [LEGACY_SIGNATURES] lista cada match con categoría `MODULE_LINK_SMELL` y captura la ruta del archivo incluido.
...
- **Archivos con eval() y código dinámico**: Se detecta `eval()` como un `SECURITY_VULN` y se marca el fragmento completo como potencialmente incompleto. El contenido del `eval()` se captura como string literal sin intentar parsearlo.
...
- **LegacySignature**: Patrón de deuda técnica detectado. Atributos: `category` del enum `SIGNATURE_CATEGORY` (`PERSISTENCE_SMELL` / `STATE_POLLUTION` / `MODULE_LINK_SMELL` / `SECURITY_VULN` / `CONSTANT_POLLUTION` / `MODERN_HYBRID`), `matched_text`, `line_number`, `severity` (critical/warning/info), y `modern_equivalent` (hint de modernización Symfony).
...
- Clasificación semántica en 6 categorías `SIGNATURE_CATEGORY` (`PERSISTENCE_SMELL`, `STATE_POLLUTION`, `MODULE_LINK_SMELL`, `SECURITY_VULN`, `CONSTANT_POLLUTION`, `MODERN_HYBRID`)
...
- Soporte para plataforma Joomla — ausente de `data/raw/multi_legacy/`; puede añadirse en iteración futura con `joomla.md` snippet y perfil de detección propio
