...
| Integration pattern | (A) Full replacement, (B) Dual path with fallback, (C) Parallel then switch | (B) | DSPy LM requires API key/config. If `dspy.LM` is not configured, fall back to existing template-based path. Zero behavioral risk. |
