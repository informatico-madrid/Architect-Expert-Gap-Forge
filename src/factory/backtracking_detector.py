...
class BacktrackingDetector:
    ...
    def detect(turns: list[Turn]) -> tuple[bool, list[int], str]:
        ...
        for i in range(len(turns) - 1):
            if (turns[i].turn_type == TurnType.ERROR and
                turns[i + 1].turn_type == TurnType.CORRECT):
                return (True, [turns[i].turn_index, turns[i + 1].turn_index], "error_recovery")
    ...
    def detect_from_messages(messages: list[Message]) -> tuple[bool, list[int], str]:
        ...
        for i in range(len(messages) - 1):
            ...
            if "error" in content_lower or "fail" in content_lower:
                ...
                if "correct" in next_content_lower or "fix" in next_content_lower or "recovery" in next_content_lower:
